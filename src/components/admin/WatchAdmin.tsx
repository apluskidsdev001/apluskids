...
import { defaultCategories, defaultVideos, type AdminCategory, type AdminVideo } from "./adminData";
...
type EditorTab = "Videos" | "Categories" | "Titles";
    ...
    <>
      <div className="flex flex-col gap-4 tablet:flex-row tablet:items-end tablet:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => { publishAdminKeys([{ draft: "aplus-admin-watch-categories", published: "aplus-published-watch-categories" }, { draft: "aplus-admin-watch-videos", published: "aplus-published-watch-videos" }, { draft: "aplus-admin-watch-copy", published: "aplus-published-watch-copy" }]); notify("Watch page published in this browser."); }} className="h-11 rounded-[12px] bg-[#238A55] px-5 text-[13px] font-medium text-white">Publish</button>
        </div>
      </div>
      ...
      <div className="mt-7 flex gap-2 overflow-x-auto border-b border-[#DCE4ED]">
        {(["Videos", "Categories", "Titles"] as EditorTab[]).map((item) => (
