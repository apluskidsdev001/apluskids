...
export default function TaskBar() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", updateTaskBarVisibility, { passive: true });
    return () => window.removeEventListener("scroll", updateTaskBarVisibility);
  }, []);
  ...
  return (
    <>
    <header
      className={`fixed left-0 right-0 top-0 z-50 hidden bg-transparent px-5 py-4 transition-transform duration-300 ease-out laptop:block desktop:px-8 ${
        isTaskBarVisible ? "translate-y-0" : "-translate-y-[calc(100%+1rem)]"
      }`}
    >
    ...
    <nav
      className={`fixed inset-x-3 bottom-3 z-50 grid h-[74px] grid-cols-5 rounded-[24px] border border-white/80 bg-white/86 px-2 shadow-[0_12px_34px_rgba(7,27,99,0.16)] backdrop-blur-xl transition-transform duration-300 ease-out laptop:hidden ${
        isTaskBarVisible ? "translate-y-0" : "translate-y-[calc(100%+1rem)]"
      }`}
    >
      {mobileTabs.map((tab) => {
