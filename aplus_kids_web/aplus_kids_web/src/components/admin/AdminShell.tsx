...
const navItems = [
  ...
  { href: "/admin/messaging", label: "Messaging Center", icon: "/icons/admin-sidebar/messaging.svg" },
  { href: "/admin/advertisements", label: "Advertisements", icon: "/icons/admin-sidebar/advertisements.svg" },
...
export default function AdminShell({ children }: { children: ReactNode }) {
  ...
  async function handleLogout(){
    ...
    setLoggingOut(true);
    try{await logout();}
    catch{/* Local sign-out still completes when the API cannot be reached. */}
        ...
        <header className="sticky top-0 z-40 h-[68px] border-b border-[#E7EDF4] bg-white/95 px-4 backdrop-blur-xl tablet:px-7 laptop:px-8">
          <div className="mx-auto flex h-full max-w-[1420px] items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => void handleLogout()} disabled={loggingOut} className="inline-flex h-9 items-center rounded-full border border-slate-200 bg-white px-3.5 text-[11px] font-bold text-slate-700 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700 disabled:cursor-wait disabled:opacity-60">{loggingOut?"Logging out…":"Log out"}</button>
            </div>
          </div>
        </header>
