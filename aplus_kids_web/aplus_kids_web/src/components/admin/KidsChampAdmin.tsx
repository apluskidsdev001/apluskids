...
import FullAccountManagementWorkspace from "./AccountManagementWorkspace";
...
const participantWorkspaceCache: {
  epoch: number;
  records: ParticipantRecord[] | null;
  recordsLoadedAt: number;
  recordsRequest: Promise<ParticipantRecord[]> | null;
  mergeHistory: ParticipantMergeRecord[] | null;
  mergeHistoryLoadedAt: number;
  mergeHistoryRequest: Promise<ParticipantMergeRecord[]> | null;
} = {
  epoch: 0,
  records: null,
  recordsLoadedAt: 0,
  recordsRequest: null,
  mergeHistory: null,
  mergeHistoryLoadedAt: 0,
  mergeHistoryRequest: null,
};

function mapParticipantRecord(item: AdminParticipantResponse): ParticipantRecord {
  return {
    reference: item.id,
    name: item.name,
    age: item.age,
    ...
    phone: item.phone,
    phoneEditable: item.phoneEditable,
...
function cacheParticipantRecords(records: ParticipantRecord[]) {
  participantWorkspaceCache.records = records;
  participantWorkspaceCache.recordsLoadedAt = Date.now();
  return records;
}

async function getParticipantRecords(force = false) {
  const cacheIsFresh = participantWorkspaceCache.records !== null
    && Date.now() - participantWorkspaceCache.recordsLoadedAt < PARTICIPANT_CACHE_TTL_MS;
  if (!force && cacheIsFresh) return participantWorkspaceCache.records as ParticipantRecord[];
  if (participantWorkspaceCache.recordsRequest) return participantWorkspaceCache.recordsRequest;

  const cacheEpoch = participantWorkspaceCache.epoch;
  const request = apiFetch("/api/v1/admin/kids-champ/participants")
    .then(async (response) => {
      if (!response.ok) throw new Error("Participants could not be loaded.");
      const records = (await response.json() as AdminParticipantResponse[]).map(mapParticipantRecord);
      return participantWorkspaceCache.epoch === cacheEpoch ? cacheParticipantRecords(records) : [];
    })
    .finally(() => {
      if (participantWorkspaceCache.recordsRequest === request) participantWorkspaceCache.recordsRequest = null;
    });
  participantWorkspaceCache.recordsRequest = request;
  return request;
}

async function getParticipantMergeHistory(force = false) {
  const cacheIsFresh = participantWorkspaceCache.mergeHistory !== null
    && Date.now() - participantWorkspaceCache.mergeHistoryLoadedAt < PARTICIPANT_CACHE_TTL_MS;
  if (!force && cacheIsFresh) return participantWorkspaceCache.mergeHistory as ParticipantMergeRecord[];
  if (participantWorkspaceCache.mergeHistoryRequest) return participantWorkspaceCache.mergeHistoryRequest;

  const cacheEpoch = participantWorkspaceCache.epoch;
  const request = apiFetch("/api/v1/admin/kids-champ/participant-merges")
    .then(async (response) => {
      const body = await response.json().catch(() => null);
      if (!response.ok || !Array.isArray(body)) throw new Error(responseErrorMessage(body) || "Merge history could not be loaded.");
      if (participantWorkspaceCache.epoch !== cacheEpoch) return [];
      participantWorkspaceCache.mergeHistory = body as ParticipantMergeRecord[];
      participantWorkspaceCache.mergeHistoryLoadedAt = Date.now();
      return participantWorkspaceCache.mergeHistory;
    })
    .finally(() => {
      if (participantWorkspaceCache.mergeHistoryRequest === request) participantWorkspaceCache.mergeHistoryRequest = null;
    });
  participantWorkspaceCache.mergeHistoryRequest = request;
  return request;
}

function preloadParticipantWorkspace() {
  void Promise.allSettled([getParticipantRecords(), getParticipantMergeHistory()]);
}

function clearParticipantWorkspaceCache() {
  participantWorkspaceCache.epoch += 1;
  participantWorkspaceCache.records = null;
  participantWorkspaceCache.recordsLoadedAt = 0;
  participantWorkspaceCache.recordsRequest = null;
  participantWorkspaceCache.mergeHistory = null;
  participantWorkspaceCache.mergeHistoryLoadedAt = 0;
  participantWorkspaceCache.mergeHistoryRequest = null;
}

type KidsChampAdminLiveUpdate = { action?: string; entityType?: string };

function parseKidsChampAdminLiveUpdate(eventBlock: string): KidsChampAdminLiveUpdate | null {
  ...
  if (!data) return null;
  try {
    return JSON.parse(data) as KidsChampAdminLiveUpdate;
  } catch {
    return null;
  }
}

function liveUpdateAffectsParticipants(update: KidsChampAdminLiveUpdate) {
  return ["SUBMISSION", "PARTICIPANT", "GUEST_PARTICIPANT", "CHILD_PROFILE", "GUEST"].includes(update.entityType ?? "");
}

function liveUpdateAffectsMerges(update: KidsChampAdminLiveUpdate) {
  return (update.action ?? "").includes("MERGE");
}

function localUpdateAffectsParticipants(path: string) {
  return /\/kids-champ\/(?:participants|submissions|guests|whatsapp\/preferences)/.test(path);
}

function localUpdateAffectsMerges(path: string) {
  return /\/kids-champ\/(?:participant-merges|guests\/merge)/.test(path);
}
...
function SubmissionsWorkspace({
  ...
  const locations = useMemo(() => [...new Set(records.map((item) => item.location))].sort(), [records]);
  ...
  async function permanentlyDeleteAllSubmissions(){
    if(backendState!=="live"){
      notify("Kids Champ records cannot be permanently deleted while the backend is unavailable.");
      return;
    }
    if(permanentlyDeleting)return;
    setPermanentlyDeleting(true);
    try{
      const response=await apiFetch("/api/v1/admin/kids-champ/submissions/permanent-all",{method:"DELETE"});
      const body=await response.json().catch(()=>null) as {deletedSubmissions?:number;deletedBatches?:number;deletedAuditEntries?:number;message?:string}|null;
      if(!response.ok){
        notify(getAdminFriendlyErrorMessage(body?.message,"permanent Kids Champ data deletion"));
        return;
      }
      const deletedSubmissions=body?.deletedSubmissions??0;
      const deletedBatches=body?.deletedBatches??0;
      const deletedAuditEntries=body?.deletedAuditEntries??0;
      setRecords([]);setSelected(new Set());setSubmissionTotalItems(0);setSubmissionTotalPages(0);setSummaryTotals({pendingReviews:0,approved:0,totalSubmissions:0});setSubmissionPage(0);
      notify(`${deletedSubmissions} Kids Champ submission${deletedSubmissions===1?"":"s"}, ${deletedBatches} ZIP batch${deletedBatches===1?"":"es"}, and ${deletedAuditEntries} audit entr${deletedAuditEntries===1?"y":"ies"} were permanently deleted.`);
      setPermanentDeleteOpen(false);
    }catch{
      notify("Kids Champ records could not be permanently deleted. No further action was taken.");
  ...
  async function approveSubmissions(ids: string[]) {
    ...
    const body = await response.json().catch(() => null) as { approvedCount?: number; alreadyApprovedCount?: number; zipWarning?: string; message?: string } | null;
    if (!response.ok) {
    ...
    notify(`${newlyApproved} approved${alreadyApproved ? `; ${alreadyApproved} already approved and ignored` : ""}.${body?.zipWarning ? ` ${body.zipWarning}` : " ZIP processing started automatically."}`);
    setSelected(new Set());
  }
  ...
  return (
    <section className="overflow-hidden rounded-[18px] border border-[#E0E8F1] bg-white shadow-[0_10px_28px_rgba(30,72,123,.05)]">
      {deleting||permanentlyDeleting ? (
        <div className="fixed inset-0 z-[310] grid place-items-center bg-[#102044]/30 backdrop-blur-[2px]" role="status" aria-live="assertive">
          <div className="flex items-center gap-4 rounded-[20px] border border-white/70 bg-white px-7 py-6 shadow-2xl">
            ...
            <div><p className="text-[15px] font-semibold text-[#172A4B]">{permanentlyDeleting?"Permanently deleting Kids Champ data":"Deleting selected submissions"}</p><p className="mt-1 text-[12px] text-[#708099]">{permanentlyDeleting?"Removing submission records, ZIP records, audit entries, and dedicated artwork files.":"Please wait while the database updates every selected record."}</p></div>
          </div>
        </div>
      ) : null}
      ...
      <div className="border-t border-[#E5EBF2]">
        ...
        <div className="flex flex-col gap-3 border-b border-[#E5EBF2] p-4 tablet:flex-row tablet:items-center tablet:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={()=>setPermanentDeleteOpen(true)} disabled={backendState!=="live"||submissionTotalItems===0||permanentlyDeleting} className="h-10 rounded-[10px] border border-red-200 bg-red-50 px-3 text-[12px] font-semibold text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-40">Permanently delete all</button>
            <button
      ...
      {permanentDeleteOpen ? (
        <ConfirmationDialog
          title="Permanently delete all Kids Champ submissions?"
          description="This cannot be undone. Every Kids Champ submission, its uploaded artwork, ZIP batch record and archive, and Kids Champ audit entry will be permanently removed. User accounts, child profiles, administrator roles, settings, messages, and other app modules will remain unchanged."
          confirmLabel="Permanently delete all"
          confirmationPhrase="DELETE ALL KIDS CHAMP"
          onCancel={() => setPermanentDeleteOpen(false)}
          onConfirm={() => void permanentlyDeleteAllSubmissions()}
        />
      ) : null}
    </section>
  );
...
function ParticipantMessageCampaign({
  ...
  async function startSending() {
    ...
    setStep("Sending");
    setRunning(true);
    const response=await apiFetch("/api/v1/admin/kids-champ/campaigns",{method:"POST",body:JSON.stringify({channel:"WHATSAPP",messageTemplate:template,participantIds:deliveries.map((item)=>item.reference),templateId:selectedTemplate.id,templateName:selectedTemplate.name,languageCode:selectedTemplate.languageCode,templateParameters,name:"Participant message",source:"PARTICIPANTS"})});
    const body=await response.json().catch(()=>null);
    ...
    setCurrentId("");
    setRunning(false);
    if(response.ok)notify("Participant campaign queued for WhatsApp delivery.");
  }
...
function ParticipantsWorkspace({
  ...
  refreshVersion: number;
  mergeRefreshVersion: number;
}) {
  const [records, setRecords] = useState<ParticipantRecord[]>(() => participantWorkspaceCache.records ?? []);
  const [recordsLoading, setRecordsLoading] = useState(() => participantWorkspaceCache.records === null);
  ...
  const [mergeSection, setMergeSection] = useState<"review" | "history">("review");
  const [mergeHistoryView, setMergeHistoryView] = useState<"active" | "all">("all");
  const [mergeHistory, setMergeHistory] = useState<ParticipantMergeRecord[]>(() => participantWorkspaceCache.mergeHistory ?? []);
  const [mergeHistoryLoading, setMergeHistoryLoading] = useState(false);
  const [mergeHistoryLoaded, setMergeHistoryLoaded] = useState(() => participantWorkspaceCache.mergeHistory !== null);
  const mergeHistoryLoadingRef = useRef(false);
  const mergeHistoryLoadedRef = useRef(participantWorkspaceCache.mergeHistory !== null);
  const [mergeSwitching, startMergeTransition] = useTransition();
  const [expandedCandidate, setExpandedCandidate] = useState<string | null>(null);
  const [mergePreview, setMergePreview] = useState<{ candidate: DuplicateGuest; targetId: string; sourceId: string } | null>(null);
  const [mergeReason, setMergeReason] = useState("");
  const [mergeSaving, setMergeSaving] = useState(false);
  const [undoMerge, setUndoMerge] = useState<ParticipantMergeRecord | null>(null);
  const [undoReason, setUndoReason] = useState("");
  const [search, setSearch] = useState("");
  const deferredSearch = useDeferredValue(search);
  const searchUpdating = search !== deferredSearch;
  ...
  useEffect(() => {
    ...
    window.addEventListener("aplus-auth-cleared", clearSensitiveState);
    return () => window.removeEventListener("aplus-auth-cleared", clearSensitiveState);
  }, []);
  const participantRefreshMounted = useRef(false);
  useEffect(() => {
    let active = true;
    const isLiveRefresh = participantRefreshMounted.current;
    participantRefreshMounted.current = true;
    const hasCachedRecords = participantWorkspaceCache.records !== null;
    const delay = window.setTimeout(() => {
      if (hasCachedRecords) setRecordsRefreshing(true);
      else setRecordsLoading(true);
      void getParticipantRecords(isLiveRefresh)
        .then((nextRecords) => {
          if (active) setRecords(nextRecords);
        })
        .catch((reason) => {
          if (active) notify(getAdminFriendlyErrorMessage(reason instanceof Error ? reason.message : undefined, "participant records"));
        })
        .finally(() => {
          if (!active) return;
  ...
  // The notice callback is intentionally excluded so only committed data updates refresh this list.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refreshVersion]);

  async function loadDuplicateGuests() {
    if (duplicatesLoading) return;
    setDuplicatesLoading(true);
    try {
      const response = await apiFetch("/api/v1/admin/kids-champ/guests/duplicate-matches");
      let matches: DuplicateGuest[];
      if (response.status === 404) {
        const [guestResponse, registeredResponse] = await Promise.all([
          apiFetch("/api/v1/admin/kids-champ/guests/duplicates"),
          apiFetch("/api/v1/admin/kids-champ/guests/registered-matches"),
        ]);
        if (!guestResponse.ok || !registeredResponse.ok) throw new Error("Duplicate participant records could not be checked.");
        matches = [
          ...await registeredResponse.json() as DuplicateGuest[],
          ...await guestResponse.json() as DuplicateGuest[],
        ];
      } else {
        if (!response.ok) throw new Error("Duplicate participant records could not be checked.");
        matches = await response.json() as DuplicateGuest[];
      }
      setDuplicateGuests(matches);
      setDuplicatesLoaded(true);
    } catch (reason) {
      notify(getAdminFriendlyErrorMessage(reason instanceof Error ? reason.message : undefined, "participant duplicate check"));
    } finally {
      setDuplicatesLoading(false);
    }
  }

  function updateParticipantRecord(updated: ParticipantRecord) {
    setRecords((current) => cacheParticipantRecords(current.map((item) => item.reference === updated.reference ? updated : item)));
  }

  function viewParticipant(reference: string) {
    const participant = records.find((item) => item.reference === reference);
    if (!participant) {
      notify("This participant record is no longer active. Refresh the merge review and try again.");
      return;
    }
    setMergePreview(null);
    openParticipant(participant, updateParticipantRecord);
  }

  async function loadMergeHistory(force = false) {
    if (mergeHistoryLoadingRef.current || (mergeHistoryLoadedRef.current && !force)) return;
    mergeHistoryLoadingRef.current = true;
    setMergeHistoryLoading(true);
    try {
      setMergeHistory(await getParticipantMergeHistory(force));
      mergeHistoryLoadedRef.current = true;
      setMergeHistoryLoaded(true);
    } catch (reason) {
      notify(getAdminFriendlyErrorMessage(reason instanceof Error ? reason.message : undefined, "participant merge history"));
    } finally {
      mergeHistoryLoadingRef.current = false;
      setMergeHistoryLoading(false);
    }
  }

  function showMergeReview() {
    startMergeTransition(() => setMergeSection("review"));
  }

  function showMergeHistory(view: "active" | "all") {
    setMergeHistoryView(view);
    startMergeTransition(() => setMergeSection("history"));
    void loadMergeHistory();
  }

  useEffect(() => {
    const delay = window.setTimeout(() => void loadMergeHistory(mergeRefreshVersion > 0), 350);
    return () => window.clearTimeout(delay);
  // Merge history is prefetched once and refreshed only for committed merge updates.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mergeRefreshVersion]);

  function openMergeReview(candidate: DuplicateGuest) {
    setMergeReason("");
    setMergePreview({ candidate, targetId: candidate.firstId, sourceId: candidate.secondId });
  }

  async function confirmMerge() {
    if (!mergePreview || !mergeReason.trim() || mergeSaving) return;
    const { candidate, targetId, sourceId } = mergePreview;
    ...
    try {
      const response = await apiFetch(`/api/v1/admin/kids-champ/guests/${registeredMatch ? "merge-registered" : "merge"}`, {
        ...
        body: JSON.stringify(registeredMatch
          ? { childId: targetId, guestId: sourceId, reason: mergeReason.trim(), matchingReasons: candidate.reasons }
          : { targetId, sourceId, reason: mergeReason.trim(), matchingReasons: candidate.reasons }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok) throw new Error(responseErrorMessage(body) || "Participant records could not be merged.");
      setRecords((current) => cacheParticipantRecords(current
        .filter((item) => item.reference !== sourceId)
        .map((item) => item.reference === targetId
      ...
      setMergePreview(null);
      setMergeReason("");
      await loadMergeHistory(true);
      notify("Participant histories merged. All submissions were preserved and the action can be undone from Merge history.");
    } catch (reason) {
      notify(getAdminFriendlyErrorMessage(reason instanceof Error ? reason.message : undefined, "participant merge"));
    } finally {
      setMergeSaving(false);
    }
  }

  async function ignoreGuestMatch(candidate: DuplicateGuest) {
    const response = await apiFetch("/api/v1/admin/kids-champ/guests/ignore-match", {
      method: "POST",
      body: JSON.stringify({ firstId: candidate.firstId, secondId: candidate.secondId }),
    });
    const body = await response.json().catch(() => null);
    ...
    notify("This possible match will be ignored.");
  }

  async function confirmUndoMerge() {
    if (!undoMerge || !undoReason.trim() || mergeSaving) return;
    setMergeSaving(true);
    try {
      const response = await apiFetch(`/api/v1/admin/kids-champ/participant-merges/${undoMerge.id}/undo`, {
        method: "POST",
        body: JSON.stringify({ reason: undoReason.trim() }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok) throw new Error(responseErrorMessage(body) || "The merge could not be undone.");
      setUndoMerge(null);
      setUndoReason("");
      await Promise.all([loadMergeHistory(true), loadDuplicateGuests()]);
      notify("The participant merge was undone and the source record was restored.");
    } catch (reason) {
      notify(getAdminFriendlyErrorMessage(reason instanceof Error ? reason.message : undefined, "participant merge undo"));
    } finally {
      setMergeSaving(false);
    }
  }
  ...
  const filteredLocationOptions = useMemo(() => {
    const query = location === "All" ? "" : location.trim().toLowerCase();
    const matchingTowns = query
      ? locations.filter((item) => item.toLowerCase().includes(query))
      : locations;
    return !query || "all".includes(query) ? ["All", ...matchingTowns] : matchingTowns;
  }, [location, locations]);
  const activeMergeCount = useMemo(() => mergeHistory.filter((item) => !item.undoneAt).length, [mergeHistory]);
  const visibleMergeHistory = useMemo(
    () => mergeHistoryView === "active" ? mergeHistory.filter((item) => !item.undoneAt) : mergeHistory,
    [mergeHistory, mergeHistoryView],
  );
  const filterMatchedParticipants = useMemo(
    () =>
      records.filter((item) => {
        ...
        const matchesLocation =
          !locationQuery || locationQuery === "all" || item.location.toLowerCase().includes(locationQuery);
  ...
  const hasActiveParticipantFilters = Boolean(
    deferredSearch.trim()
    || exactAge
    || ageMin
    || ageMax
    || (Boolean(location.trim()) && location !== "All")
    || whatsapp !== "All"
    || minSubmissions
    || dateMode !== "Any time",
  );
  const segmentCounts = useMemo(() => ({
    All: filterMatchedParticipants.length,
    Registered: filterMatchedParticipants.filter((item) => item.type === "Registered").length,
    Guest: filterMatchedParticipants.filter((item) => item.type === "Guest").length,
    Returning: filterMatchedParticipants.filter((item) => item.submissions > 1).length,
    Frequent: filterMatchedParticipants.filter((item) => item.submissions >= settings.frequentParticipantThreshold).length,
  }), [filterMatchedParticipants, settings.frequentParticipantThreshold]);
  const visible = useMemo(
    () => filterMatchedParticipants.filter((item) =>
      segment === "All"
      || item.type === segment
      || (segment === "Returning" && item.submissions > 1)
      || (segment === "Frequent" && item.submissions >= settings.frequentParticipantThreshold)),
    [filterMatchedParticipants, segment, settings.frequentParticipantThreshold],
  );
  const participantPageCount = Math.max(1, Math.ceil(visible.length / participantPageSize));
  const currentParticipantPage = Math.min(participantPage, participantPageCount - 1);
  const participantPageStart = currentParticipantPage * participantPageSize;
  const pagedParticipants = useMemo(
    () => visible.slice(participantPageStart, participantPageStart + participantPageSize),
    [participantPageSize, participantPageStart, visible],
  );
  ...
  function participantExportData() {
    const headers = [
      "Reference",
      "Child",
      "Age",
    ...
    const filters = [
      ...
      segment !== "All" ? `Participant group: ${segment}` : "",
      exactAge ? `Exact age: ${exactAge}` : "",
      ageMin ? `Minimum age: ${ageMin}` : "",
      ageMax ? `Maximum age: ${ageMax}` : "",
      location.trim() && location !== "All" ? `Home town: ${location.trim()}` : "",
      whatsapp !== "All" ? `WhatsApp: ${whatsapp}` : "",
      minSubmissions ? `Minimum submissions: ${minSubmissions}` : "",
      dateMode !== "Any time" ? `Last submission: ${dateMode}` : "",
    ].filter(Boolean).join(" | ") || "All participants (no filters)";
    return { headers, rows, filters };
  }

  function downloadParticipantExcel() {
    const { headers, rows, filters } = participantExportData();
    const generatedAt = new Date().toLocaleString("en-LK", { timeZone: "Asia/Colombo" });
    const escapeXml = (entry: string | number) => String(entry)
      .replaceAll("&", "&amp;")
      .replaceAll("<", "&lt;")
      .replaceAll(">", "&gt;")
      .replaceAll('"', "&quot;")
    ...
    const metadataRow = (label: string, value: string) =>
      `<Row><Cell ss:StyleID="MetaLabel"><Data ss:Type="String">${escapeXml(label)}</Data></Cell><Cell ss:MergeAcross="10" ss:StyleID="MetaValue"><Data ss:Type="String">${escapeXml(value)}</Data></Cell></Row>`;
    const headerRow = `<Row ss:Height="28">${headers.map((header) => `<Cell ss:StyleID="Header"><Data ss:Type="String">${escapeXml(header)}</Data></Cell>`).join("")}</Row>`;
    const dataRows = rows.map((row, rowIndex) =>
      `<Row ss:Height="22">${row.map((cell) => {
        const numeric = typeof cell === "number";
        const style = `${rowIndex % 2 ? "Alt" : "Data"}${numeric ? "Number" : "Text"}`;
        return `<Cell ss:StyleID="${style}"><Data ss:Type="${numeric ? "Number" : "String"}">${escapeXml(cell)}</Data></Cell>`;
      }).join("")}</Row>`,
    ).join("");
    const columns = [160, 150, 48, 82, 105, 118, 68, 68, 72, 100, 88, 96]
...
<Style ss:ID="Title"><Alignment ss:Vertical="Center"/><Font ss:FontName="Calibri" ss:Size="18" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#1689F7" ss:Pattern="Solid"/></Style>
<Style ss:ID="MetaLabel"><Font ss:Bold="1" ss:Color="#334155"/><Interior ss:Color="#EAF4FF" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#DCE5EF"/></Borders></Style>
<Style ss:ID="MetaValue"><Font ss:Color="#52637A"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#DCE5EF"/></Borders></Style>
<Style ss:ID="Header"><Alignment ss:Horizontal="Center" ss:Vertical="Center" ss:WrapText="1"/><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#1E6FD9" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#FFFFFF"/><Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#FFFFFF"/></Borders></Style>
<Style ss:ID="DataText"><Alignment ss:Vertical="Center" ss:WrapText="1"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#E5EBF2"/></Borders></Style>
<Style ss:ID="DataNumber"><Alignment ss:Horizontal="Center" ss:Vertical="Center"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#E5EBF2"/></Borders><NumberFormat ss:Format="0"/></Style>
<Style ss:ID="AltText"><Alignment ss:Vertical="Center" ss:WrapText="1"/><Interior ss:Color="#F7FAFE" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#E5EBF2"/></Borders></Style>
<Style ss:ID="AltNumber"><Alignment ss:Horizontal="Center" ss:Vertical="Center"/><Interior ss:Color="#F7FAFE" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#E5EBF2"/></Borders><NumberFormat ss:Format="0"/></Style>
</Styles>
<Worksheet ss:Name="Participants"><Table>${columns}<Row ss:Height="34"><Cell ss:MergeAcross="11" ss:StyleID="Title"><Data ss:Type="String">A+ Kids Champ Participant Report</Data></Cell></Row>${metadataRow("Generated", `${generatedAt} (Asia/Colombo)`)}${metadataRow("Records", `${rows.length} filtered participant${rows.length === 1 ? "" : "s"}`)}${metadataRow("Filters", filters)}<Row ss:Height="8"/>${headerRow}${dataRows}</Table><AutoFilter x:Range="R6C1:R${rows.length + 6}C12" xmlns="urn:schemas-microsoft-com:office:excel"/><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><FreezePanes/><FrozenNoSplit/><SplitHorizontal>6</SplitHorizontal><TopRowBottomPane>6</TopRowBottomPane><ProtectObjects>False</ProtectObjects><ProtectScenarios>False</ProtectScenarios></WorksheetOptions></Worksheet></Workbook>`;
    const url = URL.createObjectURL(new Blob([`\uFEFF${workbook}`], { type: "application/vnd.ms-excel" }));
    const link = document.createElement("a");
    link.href = url;
    link.download = `kids-champ-participants-${new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Colombo" }).format(new Date())}.xls`;
    link.click();
    window.setTimeout(() => URL.revokeObjectURL(url), 0);
  }

  async function downloadParticipantPdf() {
    const { headers, rows, filters } = participantExportData();
    ...
    const margin = 9;
    const columnWidths = [35, 32, 10, 18, 24, 26, 15, 15, 15, 22, 20, 20];
    ...
    const drawTableHeader = () => {
      ...
      headers.forEach((header, index) => {
        // jsPDF text drawing changes the active fill colour, so every cell must
        // restore its own background before the rectangle is painted.
        document.setFillColor(30, 111, 217);
        document.setDrawColor(255, 255, 255);
        document.rect(x, y, columnWidths[index], 8, "FD");
        document.setTextColor(255, 255, 255);
        document.text(header, x + 1.2, y + 5, { maxWidth: columnWidths[index] - 2.4 });
        x += columnWidths[index];
      });
      y += 8;
    };
    const drawPageHeading = (continued = false) => {
      document.setTextColor(22, 50, 82);
      document.setFont("helvetica", "bold");
      document.setFontSize(15);
      document.text(`A+ Kids Champ Participant Report${continued ? " (continued)" : ""}`, margin, 11);
      document.setFont("helvetica", "normal");
      document.setFontSize(7.5);
      document.setTextColor(82, 99, 122);
      if (!continued) {
        document.text(`Generated: ${new Date().toLocaleString("en-LK", { timeZone: "Asia/Colombo" })} (Asia/Colombo)`, margin, 17);
        document.text(`Records: ${rows.length}`, margin, 21);
        const filterLines = document.splitTextToSize(`Filters: ${filters}`, pageWidth - (margin * 2)).slice(0, 2);
        document.text(filterLines, margin, 25);
        y = 27 + (filterLines.length * 3.3);
      } else {
        y = 15;
      }
      drawTableHeader();
    ...
    rows.forEach((row, rowIndex) => {
      const cellLines = row.map((cell, index) => document.splitTextToSize(String(cell), columnWidths[index] - 2.4).slice(0, 2));
      const rowHeight = Math.max(7, Math.max(...cellLines.map((lines) => lines.length)) * 3.1 + 2);
      if (y + rowHeight > pageHeight - 10) {
        document.addPage();
        drawPageHeading(true);
      }
      let x = tableLeft;
      const rowFill: [number, number, number] = rowIndex % 2 ? [247, 250, 254] : [255, 255, 255];
      document.setFont("helvetica", "normal");
      document.setFontSize(5.8);
      cellLines.forEach((lines, index) => {
        document.setFillColor(...rowFill);
        document.setDrawColor(221, 229, 239);
        document.rect(x, y, columnWidths[index], rowHeight, "FD");
        document.setTextColor(52, 65, 85);
        document.text(lines, x + 1.2, y + 3.6, { maxWidth: columnWidths[index] - 2.4 });
        x += columnWidths[index];
      });
      y += rowHeight;
    });
    ...
    for (let page = 1; page <= pageCount; page += 1) {
      ...
      document.setTextColor(113, 128, 150);
      document.setFontSize(7);
      document.text(`Page ${page} of ${pageCount}`, pageWidth - margin, pageHeight - 4, { align: "right" });
    }
    document.save(`kids-champ-participants-${new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Colombo" }).format(new Date())}.pdf`);
  }

  async function exportParticipants(format: "pdf" | "excel") {
    if (!visible.length || exportingFormat) {
      if (!visible.length) notify("There are no filtered participant records to export.");
      return;
    }
    setExportingFormat(format);
    await new Promise<void>((resolve) => window.requestAnimationFrame(() => resolve()));
    try {
      if (format === "pdf") await downloadParticipantPdf();
      else downloadParticipantExcel();
      setExportDialogOpen(false);
      notify(`${visible.length} filtered participant record${visible.length === 1 ? "" : "s"} exported as ${format === "pdf" ? "PDF" : "Excel"}.`);
    } catch {
      notify(`The ${format === "pdf" ? "PDF" : "Excel"} file could not be created. Please try again.`);
    } finally {
  ...
  return (
    <section className="space-y-3">
      ...
      <section className="rounded-[14px] border border-amber-200 bg-[#FFF9EA] p-4 tablet:p-5 desktop:p-6">
        <div className="flex flex-col items-start justify-between gap-4 laptop:flex-row">
          <div className="flex gap-3">
            <span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#FFE9AE] text-lg text-amber-700">!</span>
            <div><h3 className="text-[16px] font-semibold text-amber-950">Participant merge centre</h3>
            <p className="mt-1 max-w-3xl text-[12px] leading-5 text-amber-800">Compare both records, check why they matched, and choose exactly which record receives the history. Every merge is audited and can be undone.</p>
            </div>
          </div>
          {mergeSwitching ? <span className="inline-flex shrink-0 items-center gap-2 rounded-full bg-white px-3 py-2 text-[10px] font-semibold text-amber-900" role="status"><span className="size-3 animate-spin rounded-full border-2 border-amber-200 border-t-amber-800" />Opening view...</span> : null}
        </div>
        <div className="mt-5 grid grid-cols-1 gap-3 tablet:grid-cols-3 laptop:gap-4 desktop:gap-5 monitor:gap-6">
          <button type="button" onClick={showMergeReview} aria-pressed={mergeSection === "review"} className={`group min-h-28 rounded-[14px] border p-4 text-left transition laptop:min-h-32 laptop:p-5 desktop:min-h-36 desktop:p-6 monitor:p-7 ${mergeSection === "review" ? "border-amber-700 bg-amber-900 text-white shadow-[0_10px_24px_rgba(120,53,15,.2)]" : "border-amber-200 bg-white text-amber-950 hover:-translate-y-0.5 hover:border-amber-400"}`}>
            <span className={`text-[10px] font-bold uppercase tracking-[.12em] ${mergeSection === "review" ? "text-amber-200" : "text-amber-700"}`}>Needs review</span>
            <strong className="mt-2 block text-[28px] leading-none">{duplicatesLoaded ? duplicateGuests.length : "—"}</strong>
            <span className={`mt-2 block text-[10px] leading-4 ${mergeSection === "review" ? "text-amber-100" : "text-[#75839A]"}`}>{duplicatesLoading ? "Checking suggested matches..." : duplicatesLoaded ? `${duplicateGuests.length} suggested match${duplicateGuests.length === 1 ? "" : "es"} awaiting a decision` : "Not checked yet"}</span>
          </button>
          <button type="button" onClick={() => showMergeHistory("active")} aria-pressed={mergeSection === "history" && mergeHistoryView === "active"} className={`group min-h-28 rounded-[14px] border p-4 text-left transition laptop:min-h-32 laptop:p-5 desktop:min-h-36 desktop:p-6 monitor:p-7 ${mergeSection === "history" && mergeHistoryView === "active" ? "border-emerald-600 bg-emerald-600 text-white shadow-[0_10px_24px_rgba(5,150,105,.18)]" : "border-emerald-200 bg-white text-emerald-950 hover:-translate-y-0.5 hover:border-emerald-400"}`}>
            <span className={`text-[10px] font-bold uppercase tracking-[.12em] ${mergeSection === "history" && mergeHistoryView === "active" ? "text-emerald-100" : "text-emerald-700"}`}>Active merges</span>
            <strong className="mt-2 block text-[28px] leading-none">{mergeHistoryLoaded ? activeMergeCount : "—"}</strong>
            <span className={`mt-2 block text-[10px] leading-4 ${mergeSection === "history" && mergeHistoryView === "active" ? "text-emerald-100" : "text-[#75839A]"}`}>{mergeHistoryLoading && !mergeHistoryLoaded ? "Loading merge count..." : `${activeMergeCount} merge${activeMergeCount === 1 ? "" : "s"} available to undo`}</span>
          </button>
          <button type="button" onClick={() => showMergeHistory("all")} aria-pressed={mergeSection === "history" && mergeHistoryView === "all"} className={`group min-h-28 rounded-[14px] border p-4 text-left transition laptop:min-h-32 laptop:p-5 desktop:min-h-36 desktop:p-6 monitor:p-7 ${mergeSection === "history" && mergeHistoryView === "all" ? "border-blue-600 bg-blue-600 text-white shadow-[0_10px_24px_rgba(37,99,235,.18)]" : "border-blue-200 bg-white text-blue-950 hover:-translate-y-0.5 hover:border-blue-400"}`}>
            <span className={`text-[10px] font-bold uppercase tracking-[.12em] ${mergeSection === "history" && mergeHistoryView === "all" ? "text-blue-100" : "text-blue-700"}`}>Merge history</span>
            <strong className="mt-2 block text-[28px] leading-none">{mergeHistoryLoaded ? mergeHistory.length : "—"}</strong>
            <span className={`mt-2 block text-[10px] leading-4 ${mergeSection === "history" && mergeHistoryView === "all" ? "text-blue-100" : "text-[#75839A]"}`}>{mergeHistoryLoading && !mergeHistoryLoaded ? "Loading audit history..." : `${mergeHistory.length} total audited merge${mergeHistory.length === 1 ? "" : "s"}`}</span>
          </button>
        </div>
        {mergeSection === "review" ? <>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-3">
          <p className="text-[12px] text-[#738096]">Duplicate analysis runs only when requested, so the participant list stays fast.</p>
          <button type="button" onClick={() => void loadDuplicateGuests()} disabled={duplicatesLoading} className="rounded-[10px] border border-amber-300 bg-white px-3 py-2 text-[11px] font-semibold text-amber-900 hover:bg-amber-50 disabled:cursor-wait disabled:opacity-60">{duplicatesLoading ? "Checking..." : duplicatesLoaded ? "Check again" : "Check duplicates"}</button>
        </div>
        {duplicatesLoading ? <div className="mt-4 space-y-3" role="status" aria-label="Checking participant matches">
          {[0, 1].map((item) => <div key={item} className="animate-pulse rounded-[14px] border border-amber-100 bg-white p-4"><div className="h-3 w-2/5 rounded bg-amber-100" /><div className="mt-3 h-2.5 w-3/4 rounded bg-slate-100" /><div className="mt-4 flex gap-2"><span className="h-6 w-24 rounded-full bg-amber-100" /><span className="h-6 w-32 rounded-full bg-amber-100" /></div></div>)}
          <p className="sr-only">Checking possible matches in the background.</p>
        </div> : null}
        {!duplicatesLoaded && !duplicatesLoading ? <p className="mt-4 rounded-xl bg-white p-4 text-[12px] text-[#738096]">Duplicate analysis runs only when requested, so it no longer delays the participant list.</p> : null}
        {duplicatesLoaded && !duplicatesLoading && duplicateGuests.length ? <div className="mt-4 space-y-3">{duplicateGuests.map((candidate) => (
          <article key={`${candidate.firstId}-${candidate.secondId}`} className="rounded-[14px] border border-amber-200 bg-white p-4">
            <div className="flex flex-col gap-4 tablet:flex-row tablet:items-center tablet:justify-between">
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                <button type="button" onClick={() => viewParticipant(candidate.firstId)} className="rounded-[10px] border border-blue-200 bg-blue-50 px-3 py-2.5 text-[11px] font-semibold text-blue-700">View first</button>
                <button type="button" onClick={() => viewParticipant(candidate.secondId)} className="rounded-[10px] border border-blue-200 bg-blue-50 px-3 py-2.5 text-[11px] font-semibold text-blue-700">View second</button>
                <button type="button" onClick={() => setExpandedCandidate(expandedCandidate === `${candidate.firstId}-${candidate.secondId}` ? null : `${candidate.firstId}-${candidate.secondId}`)} className="rounded-[10px] border border-slate-300 bg-white px-3 py-2.5 text-[11px] font-semibold text-slate-700">Compare</button>
                <button type="button" onClick={() => openMergeReview(candidate)} className="rounded-[10px] bg-emerald-600 px-3 py-2.5 text-[11px] font-semibold text-white hover:bg-emerald-700">Review merge</button>
                <button type="button" onClick={() => void ignoreGuestMatch(candidate)} className="rounded-[10px] border border-slate-300 bg-white px-3 py-2.5 text-[11px] font-semibold text-slate-700 hover:bg-slate-50">Not a match</button>
              </div>
            </div>
            {expandedCandidate === `${candidate.firstId}-${candidate.secondId}` ? <div className="mt-3 grid gap-3 rounded-xl bg-slate-50 p-3 tablet:grid-cols-2">
              {[
                { label: "First record", name: candidate.firstName, type: candidate.firstType, parent: candidate.firstParentName, phone: candidate.firstPhone, town: candidate.firstHometown, dob: candidate.firstDateOfBirth },
                { label: "Second record", name: candidate.secondName, type: candidate.secondType, parent: candidate.secondParentName, phone: candidate.secondPhone, town: candidate.secondHometown, dob: candidate.secondDateOfBirth },
              ].map((details) => <dl key={details.label} className="grid grid-cols-2 gap-2 rounded-lg bg-white p-3 text-[11px]"><div className="col-span-2"><dt className="font-bold uppercase text-[#8490A2]">{details.label}</dt><dd className="mt-1 font-semibold">{details.name} · {details.type}</dd></div><div><dt className="text-[#8490A2]">Date of birth</dt><dd className="font-semibold">{details.dob}</dd></div><div><dt className="text-[#8490A2]">Home town</dt><dd className="font-semibold">{details.town}</dd></div><div><dt className="text-[#8490A2]">Guardian</dt><dd className="font-semibold">{details.parent || "Not recorded"}</dd></div><div><dt className="text-[#8490A2]">Mobile</dt><dd className="font-semibold">{details.phone}</dd></div></dl>)}
            </div> : null}
          </article>
        ))}</div> : null}
        {duplicatesLoaded && !duplicatesLoading && !duplicateGuests.length ? <p className="mt-4 rounded-xl bg-white p-4 text-[12px] text-[#738096]">No likely duplicate participant records were found.</p> : null}
        </> : <div className="mt-4 space-y-3">
          {mergeHistoryLoading && !mergeHistoryLoaded ? <div className="space-y-3" role="status" aria-label="Loading merge history">{[0, 1, 2].map((item) => <div key={item} className="animate-pulse rounded-xl border border-amber-100 bg-white p-4"><div className="h-3 w-1/3 rounded bg-slate-200" /><div className="mt-3 h-2.5 w-2/3 rounded bg-slate-100" /><div className="mt-4 h-12 rounded-lg bg-slate-50" /></div>)}</div> : null}
          {!mergeHistoryLoading && mergeHistoryLoaded && !visibleMergeHistory.length ? <div className="rounded-xl bg-white p-4 text-[12px] text-[#738096]">{mergeHistoryView === "active" ? "No active merges are available to undo." : "No participant merges have been recorded."}</div> : null}
          {visibleMergeHistory.map((entry) => <article key={entry.id} className="rounded-xl border border-amber-200 bg-white p-4">
            <div className="flex flex-col gap-3 tablet:flex-row tablet:items-start tablet:justify-between"><div><p className="text-[13px] font-semibold text-[#263852]">{entry.sourceName} <span className="text-amber-700">→</span> {entry.targetName}</p><p className="mt-1 text-[11px] text-[#738096]">{entry.movedSubmissions} submission{entry.movedSubmissions === 1 ? "" : "s"} · {new Date(entry.mergedAt).toLocaleString()} · {entry.mergedBy}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${entry.undoneAt ? "bg-slate-100 text-slate-700" : "bg-emerald-100 text-emerald-800"}`}>{entry.undoneAt ? "Undone" : "Active merge"}</span></div>
            <p className="mt-3 rounded-lg bg-slate-50 p-3 text-[11px] leading-5 text-[#52637A]"><strong>Admin reason:</strong> {entry.reason}</p>
            {entry.matchingReasons.length ? <div className="mt-2 flex flex-wrap gap-1.5">{entry.matchingReasons.map((reason) => <span key={reason} className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-semibold text-amber-900">{reason}</span>)}</div> : null}
            {entry.undoneAt ? <p className="mt-2 text-[11px] text-[#738096]">Undone by {entry.undoneBy}: {entry.undoReason}</p> : <button type="button" onClick={() => { setUndoMerge(entry); setUndoReason(""); }} className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-[10px] font-semibold text-red-700">Undo merge</button>}
          </article>)}
        </div>}
      </section>
      <div className="overflow-hidden rounded-[18px] border border-[#E0E7EF] bg-white shadow-[0_10px_28px_rgba(30,72,123,.05)]">
        ...
        {filtersOpen ? (
          <div className="grid gap-3 border-b border-[#E5EBF2] bg-[#F8FAFC] p-4 tablet:grid-cols-3 desktop:grid-cols-6">
            ...
            <div className="relative text-[10px] font-semibold uppercase text-[#748197]">
              <label htmlFor="kids-champ-participant-location">Home town</label>
              <input
                id="kids-champ-participant-location"
                type="text"
                value={location}
                onChange={(event) => {
                  ...
                  setActiveLocationOption(0);
                }}
                onFocus={(event) => {
                  if (location === "All") event.currentTarget.select();
                  setLocationOptionsOpen(true);
                  setActiveLocationOption(0);
                }}
                onClick={(event) => {
                ...
                onBlur={() => {
                  if (!location.trim()) selectLocation("All");
                  else setLocationOptionsOpen(false);
                }}
                onKeyDown={(event) => {
                  if (event.key === "ArrowDown") {
                    event.preventDefault();
                    setLocationOptionsOpen(true);
                    setActiveLocationOption((current) => Math.min(filteredLocationOptions.length - 1, current + 1));
                  } else if (event.key === "ArrowUp") {
                    event.preventDefault();
                    setLocationOptionsOpen(true);
                    setActiveLocationOption((current) => Math.max(0, current - 1));
                  } else if (event.key === "Enter" && locationOptionsOpen && activeLocationOption >= 0) {
                    event.preventDefault();
                    const option = filteredLocationOptions[activeLocationOption];
                ...
                autoComplete="off"
                placeholder="Type a home town"
                className={`${fieldClass} mt-1`}
                aria-label="Filter participants by home town"
                role="combobox"
                aria-autocomplete="list"
                aria-expanded={locationOptionsOpen}
                aria-controls="kids-champ-participant-location-options"
                aria-activedescendant={activeLocationOption >= 0 && filteredLocationOptions[activeLocationOption] ? `kids-champ-location-option-${activeLocationOption}` : undefined}
              />
              {locationOptionsOpen ? (
                <div
                  id="kids-champ-participant-location-options"
                  role="listbox"
                  aria-label="Available home towns"
                  className="absolute left-0 right-0 top-full z-40 mt-1 max-h-52 overflow-y-auto overscroll-contain rounded-[10px] border border-[#C9D8EA] bg-white p-1 normal-case shadow-[0_12px_30px_rgba(30,72,123,.16)]"
                >
                  {filteredLocationOptions.length ? filteredLocationOptions.map((item, index) => (
                    <button
                      key={item}
                      id={`kids-champ-location-option-${index}`}
                      type="button"
                      role="option"
                      aria-selected={location === item}
                      onMouseDown={(event) => event.preventDefault()}
                      onMouseEnter={() => setActiveLocationOption(index)}
                      onClick={() => selectLocation(item)}
                      className={`block w-full rounded-[7px] px-3 py-2 text-left text-[11px] font-medium transition ${activeLocationOption === index ? "bg-[#2488F4] text-white" : "text-[#53657D] hover:bg-blue-50 hover:text-blue-700"}`}
                    >
                      {item}
                    </button>
                  )) : (
                    <p className="px-3 py-3 text-[11px] font-medium text-[#8490A2]">No matching home towns.</p>
                  )}
                </div>
              ) : null}
            </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-b border-[#E5EBF2] p-3 tablet:grid-cols-3 tablet:p-4 laptop:grid-cols-5 desktop:gap-3 monitor:p-5">
          {(Object.keys(segmentCounts) as Array<keyof typeof segmentCounts>).map((item) => {
            const description = item === "All"
              ? hasActiveParticipantFilters
                ? `${records.length} total participants`
                : "all participants"
              : hasActiveParticipantFilters
                ? `${item.toLowerCase()} matches within filters`
                : item === "Returning"
                  ? "More than one submission"
                  : item === "Frequent"
                    ? `${settings.frequentParticipantThreshold}+ submissions`
                    : `${item.toLowerCase()} participants`;
            return (
            <button
              key={item}
              onClick={() => {
              ...
              className={`min-h-20 rounded-[13px] border p-3 text-left transition desktop:p-4 ${segment === item ? "border-[#2488F4] bg-[#2488F4] text-white shadow-[0_8px_20px_rgba(36,136,244,.2)]" : "border-[#DCE5EF] bg-[#F8FAFC] text-[#53657D] hover:border-blue-200 hover:bg-blue-50"}`}
            >
              <span className="flex flex-wrap items-center gap-1.5 text-[11px] font-semibold monitor:text-[12px]">
                {item}
                {item === "All" && hasActiveParticipantFilters ? (
                  <span className={`rounded-full px-2 py-0.5 text-[8px] font-bold uppercase tracking-wide ${segment === item ? "bg-white/20 text-white" : "bg-blue-100 text-blue-700"}`}>Filtered</span>
                ) : null}
              </span>
              <strong className="mt-1 block text-[22px] leading-none desktop:text-[25px]">{segmentCounts[item]}</strong>
              <span className={`mt-2 block text-[9px] leading-4 desktop:text-[10px] ${segment === item ? "text-blue-100" : "text-[#8793A5]"}`}>{description}</span>
            </button>
          );})}
        </div>
        {selected.size ? (
        ...
        <div className="overflow-x-auto">
              ...
              {pagedParticipants.map((item) => {
                const open = () =>
                  openParticipant(item, (updated) =>
                    setRecords((current) =>
                      cacheParticipantRecords(current.map((record) =>
          ...
          {recordsLoading && !records.length ? (
            <div className="flex items-center justify-center gap-2 py-14 text-[13px] text-[#66758B]" role="status">
              <span className="size-5 animate-spin rounded-full border-2 border-blue-100 border-t-[#2488F4]" />
              Loading participant records...
            </div>
          ) : null}
          {!recordsLoading && !visible.length ? (
            <div className="py-14 text-center text-[13px] text-[#8490A2]">
              No participants match the selected filters.
            </div>
          ) : null}
        </div>
        <div className="flex flex-col gap-3 border-t border-[#E5EBF2] px-3 py-3 tablet:flex-row tablet:items-center tablet:justify-between tablet:px-4 laptop:gap-4 desktop:px-5 monitor:py-4">
          <div className="flex flex-col gap-1 tablet:min-w-[190px]">
            <p className="text-[12px] text-[#8490A2]">
              {visible.length
                ? `Showing ${participantPageStart + 1}-${Math.min(participantPageStart + participantPageSize, visible.length)} of ${visible.length} matching participants`
                : "No matching participants"}
            </p>
            <span className="text-[10px] text-[#9AA6B7]">Participant details are cached only for this signed-in page session.</span>
          </div>
          <div className="flex flex-wrap items-center gap-2 tablet:justify-end">
            <label className="flex items-center gap-2 text-[10px] font-semibold text-[#65758B]">
              Rows
              <select
                ...
                className="h-9 rounded-[9px] border border-[#DCE5EF] bg-white px-2 text-[11px] text-[#40516B]"
            ...
            <button
              type="button"
              onClick={() => setParticipantPage((current) => Math.max(0, current - 1))}
              disabled={currentParticipantPage === 0}
              className="h-9 rounded-[9px] border border-[#DCE5EF] bg-white px-3 text-[10px] font-semibold text-[#53657D] disabled:cursor-not-allowed disabled:opacity-40"
            >
              Previous
            </button>
            <span className="min-w-16 text-center text-[10px] font-semibold text-[#53657D]">
              {currentParticipantPage + 1} / {participantPageCount}
            </span>
            <button
              type="button"
              onClick={() => setParticipantPage((current) => Math.min(participantPageCount - 1, current + 1))}
              disabled={currentParticipantPage >= participantPageCount - 1}
              className="h-9 rounded-[9px] border border-[#DCE5EF] bg-white px-3 text-[10px] font-semibold text-[#53657D] disabled:cursor-not-allowed disabled:opacity-40"
            >
              Next
            </button>
            <span className={`rounded-full px-3 py-1.5 text-[10px] font-semibold ${recordsLoading || recordsRefreshing || searchUpdating ? "bg-blue-50 text-blue-700" : "bg-emerald-50 text-emerald-700"}`} role="status" aria-live="polite">
              {recordsLoading ? "Loading..." : recordsRefreshing ? "Syncing..." : searchUpdating ? "Filtering..." : "Session cache ready"}
            </span>
          </div>
        </div>
      </div>
      {exportDialogOpen ? (
        <div className="fixed inset-0 z-[110] grid place-items-center bg-slate-950/45 p-3 tablet:p-5 laptop:p-6 desktop:p-8 monitor:p-10" role="dialog" aria-modal="true" aria-labelledby="participant-export-title">
          <section className="w-full max-w-xl rounded-[20px] bg-white p-4 shadow-2xl tablet:p-5 laptop:max-w-2xl laptop:p-6 desktop:p-7 monitor:max-w-3xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 id="participant-export-title" className="text-[19px] font-semibold text-[#1D2D45]">Export filtered participants</h3>
                <p className="mt-1 text-[12px] leading-5 text-[#718096]">Choose a file format for all {visible.length} matching participant record{visible.length === 1 ? "" : "s"}.</p>
              </div>
              <button type="button" onClick={() => setExportDialogOpen(false)} disabled={Boolean(exportingFormat)} className="grid size-9 shrink-0 place-items-center rounded-full bg-slate-100 text-slate-600 disabled:opacity-40" aria-label="Close export options">×</button>
            </div>
            <div className="mt-5 grid grid-cols-1 gap-3 tablet:grid-cols-2 laptop:gap-4 desktop:gap-5 monitor:mt-6">
              <button type="button" onClick={() => void exportParticipants("excel")} disabled={Boolean(exportingFormat)} className="rounded-[15px] border border-emerald-200 bg-emerald-50 p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-wait disabled:opacity-60 tablet:p-5">
                <span className="grid size-10 place-items-center rounded-[11px] bg-emerald-600 text-[12px] font-bold text-white">XLS</span>
                <strong className="mt-4 block text-[15px] text-emerald-900">Excel workbook</strong>
                <span className="mt-1 block text-[11px] leading-5 text-emerald-800">Structured report with title, filter details, styled headers, column widths, alternating rows, frozen headings and Excel filtering.</span>
                <span className="mt-3 block text-[11px] font-semibold text-emerald-700">{exportingFormat === "excel" ? "Creating Excel file..." : "Export as Excel"}</span>
              </button>
              <button type="button" onClick={() => void exportParticipants("pdf")} disabled={Boolean(exportingFormat)} className="rounded-[15px] border border-red-200 bg-red-50 p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-wait disabled:opacity-60 tablet:p-5">
                <span className="grid size-10 place-items-center rounded-[11px] bg-red-600 text-[11px] font-bold text-white">PDF</span>
                <strong className="mt-4 block text-[15px] text-red-900">PDF report</strong>
                <span className="mt-1 block text-[11px] leading-5 text-red-800">Print-ready landscape report with filter details, repeated table headings, alternating rows and numbered pages.</span>
                <span className="mt-3 block text-[11px] font-semibold text-red-700">{exportingFormat === "pdf" ? "Creating PDF file..." : "Export as PDF"}</span>
              </button>
            </div>
            <div className="mt-5 flex justify-end">
              <button type="button" onClick={() => setExportDialogOpen(false)} disabled={Boolean(exportingFormat)} className={secondaryButton}>Cancel</button>
            </div>
          </section>
        </div>
      ) : null}
      {mergePreview ? <div className="fixed inset-0 z-[90] grid place-items-center bg-slate-950/45 p-3 tablet:p-6" role="dialog" aria-modal="true" aria-labelledby="merge-preview-title">
        <section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[18px] bg-white p-4 shadow-2xl tablet:p-6 desktop:p-7">
          <div className="flex items-start justify-between gap-4"><div><h3 id="merge-preview-title" className="text-[19px] font-semibold text-[#1D2D45]">Confirm participant merge</h3><p className="mt-1 text-[12px] leading-5 text-[#718096]">The source identity will be hidden, but its submissions and merge history will remain preserved.</p></div><button type="button" onClick={() => setMergePreview(null)} className="grid size-9 shrink-0 place-items-center rounded-full bg-slate-100 text-slate-600" aria-label="Close merge review">×</button></div>
          <div className="mt-5 grid gap-3 laptop:grid-cols-[1fr_auto_1fr] laptop:items-stretch">
            {[{ label: "Source record", id: mergePreview.sourceId }, { label: "Destination record", id: mergePreview.targetId }].map((side, index) => {
              const first = side.id === mergePreview.candidate.firstId;
              return <div key={side.label} className={`rounded-xl border p-4 ${index === 0 ? "border-blue-200 bg-blue-50" : "border-emerald-200 bg-emerald-50"}`}><p className="text-[10px] font-bold uppercase tracking-wide text-[#718096]">{side.label}</p><p className="mt-1 text-[15px] font-semibold">{first ? mergePreview.candidate.firstName : mergePreview.candidate.secondName}</p><p className="mt-1 text-[11px] text-[#607089]">{first ? mergePreview.candidate.firstType : mergePreview.candidate.secondType} · {first ? mergePreview.candidate.firstSubmissions : mergePreview.candidate.secondSubmissions} submissions</p><button type="button" onClick={() => viewParticipant(side.id)} className="mt-3 rounded-lg border border-current px-2.5 py-1.5 text-[10px] font-semibold text-blue-700">View / edit separately</button></div>;
            })}
            <div className="hidden items-center justify-center text-2xl text-amber-700 laptop:flex">→</div>
          </div>
          {mergePreview.candidate.matchType === "GUEST_GUEST" ? <button type="button" onClick={() => setMergePreview((current) => current ? { ...current, targetId: current.sourceId, sourceId: current.targetId } : current)} className="mt-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-[10px] font-semibold text-blue-700">Swap source and destination</button> : <p className="mt-3 rounded-lg bg-blue-50 p-3 text-[11px] text-blue-800">Registered profiles remain the destination; the guest submission history is moved into that child profile.</p>}
          <div className="mt-4"><p className="text-[10px] font-bold uppercase text-[#718096]">Why the system suggested this match</p><div className="mt-2 flex flex-wrap gap-1.5">{mergePreview.candidate.reasons.map((reason) => <span key={reason} className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-semibold text-amber-900">{reason}</span>)}</div></div>
          <label className="mt-4 block text-[10px] font-bold uppercase text-[#718096]">Admin merge reason <span className="text-red-600">required</span><textarea value={mergeReason} onChange={(event) => setMergeReason(event.target.value)} maxLength={600} rows={3} className={`${fieldClass} mt-2 h-auto resize-y py-3`} placeholder="Explain why these records represent the same child and should be merged." /></label>
          <div className="mt-5 flex flex-col-reverse gap-2 tablet:flex-row tablet:justify-end"><button type="button" onClick={() => setMergePreview(null)} className={secondaryButton}>Cancel</button><button type="button" onClick={() => void confirmMerge()} disabled={!mergeReason.trim() || mergeSaving} className={`${primaryButton} disabled:cursor-not-allowed disabled:opacity-40`}>{mergeSaving ? "Merging..." : "Merge source into destination"}</button></div>
        </section>
      </div> : null}
      {undoMerge ? <div className="fixed inset-0 z-[90] grid place-items-center bg-slate-950/45 p-3 tablet:p-6" role="dialog" aria-modal="true" aria-labelledby="undo-merge-title"><section className="w-full max-w-lg rounded-[18px] bg-white p-4 shadow-2xl tablet:p-6"><h3 id="undo-merge-title" className="text-[18px] font-semibold">Undo participant merge</h3><p className="mt-2 text-[12px] leading-5 text-[#718096]">Restore <strong>{undoMerge.sourceName}</strong> as a separate participant and move only the submissions recorded by this merge back to it.</p><label className="mt-4 block text-[10px] font-bold uppercase text-[#718096]">Undo reason <span className="text-red-600">required</span><textarea value={undoReason} onChange={(event) => setUndoReason(event.target.value)} maxLength={600} rows={3} className={`${fieldClass} mt-2 h-auto resize-y py-3`} /></label><div className="mt-5 flex flex-col-reverse gap-2 tablet:flex-row tablet:justify-end"><button type="button" onClick={() => setUndoMerge(null)} className={secondaryButton}>Cancel</button><button type="button" onClick={() => void confirmUndoMerge()} disabled={!undoReason.trim() || mergeSaving} className="rounded-[10px] bg-red-600 px-4 py-2.5 text-[12px] font-semibold text-white disabled:opacity-40">{mergeSaving ? "Restoring..." : "Undo merge"}</button></div></section></div> : null}
      {campaignTargets ? (
...
function ParticipantDetailsEditor({
  ...
  const [saving, setSaving] = useState(false);
  async function save() {
    if (!draft.name.trim() || !draft.dateOfBirth || !draft.location.trim() || saving) return;
    setSaving(true);
    try {
      const response = await apiFetch(`/api/v1/admin/kids-champ/participants/${draft.reference}`, {
        method: "PATCH",
        body: JSON.stringify({ name: draft.name.trim(), dateOfBirth: draft.dateOfBirth, hometown: draft.location.trim(), phone: draft.phone.trim() }),
      });
      const body = await response.json().catch(() => null) as null | {
        id: string; name: string; age: number; dateOfBirth: string; type: string; location: string; phone: string; phoneEditable: boolean;
        submissions: number; approved: number; telecasted: number; joinedAt: string; lastSubmissionAt: string; whatsappConsented: boolean;
      };
      if (!response.ok || !body) throw new Error(responseErrorMessage(body) || "Participant details could not be saved.");
      const updated: ParticipantRecord = {
        ...
        phone: body.phone,
        phoneEditable: body.phoneEditable,
        submissions: body.submissions,
        approved: body.approved,
        telecasted: body.telecasted,
        joinedDate: body.joinedAt.slice(0, 10),
        lastSubmissionDate: body.lastSubmissionAt.slice(0, 10),
      };
      setDraft(updated);
      onSave?.(updated);
      setEditing(false);
      notify("Participant details saved to the database.");
    } catch (reason) {
      notify(getAdminFriendlyErrorMessage(reason instanceof Error ? reason.message : undefined, "participant details"));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div>
      ...
      {editing ? (
        <div className="mt-5 grid grid-cols-2 gap-3">
          ...
          <label className="text-[10px] font-semibold uppercase text-[#8793A5]">
            Type
            <span className="mt-1 block rounded-[10px] bg-slate-100 px-3 py-3 text-[12px] normal-case text-[#52637A]">{draft.type} · identity type cannot be changed here</span>
          </label>
          <label className="text-[10px] font-semibold uppercase text-[#8793A5]">
            Phone
            <input
              ...
              readOnly={!draft.phoneEditable}
              className={`${fieldClass} mt-1 ${draft.phoneEditable ? "" : "cursor-not-allowed bg-slate-100 text-slate-500"}`}
            />
            {!draft.phoneEditable ? <span className="mt-1 block text-[9px] normal-case text-[#8793A5]">Registered account phone numbers are managed in Account & Management.</span> : null}
          </label>
          ...
          <label className="text-[10px] font-semibold uppercase text-[#8793A5]">
            ...
            <input
              ...
              readOnly
              className={`${fieldClass} mt-1 cursor-not-allowed bg-slate-100 text-slate-500`}
            />
          </label>
          <label className="text-[10px] font-semibold uppercase text-[#8793A5]">
            Submissions
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        {editing ? (
          <>
            <button onClick={() => void save()} disabled={saving || !draft.name.trim() || !draft.dateOfBirth || !draft.location.trim()} className={`${primaryButton} disabled:cursor-not-allowed disabled:opacity-40`}>
  ...
  if (drawer.participant)
    return (
      <ParticipantDetailsEditor
        key={drawer.participant.reference}
...
export function AccountManagementAdminPage() {
  ...
  return <>
    ...
    <div className="mt-6"><FullAccountManagementWorkspace notify={notify}/></div>
...
export default function KidsChampAdmin() {
  ...
  const [workspaceSwitching, startWorkspaceTransition] = useTransition();
  ...
  const [participantRefreshVersion, setParticipantRefreshVersion] = useState(0);
  const [mergeRefreshVersion, setMergeRefreshVersion] = useState(0);
            ...
            while (!controller.signal.aborted) {
              ...
              if (updateEvents.length) {
                const updates = updateEvents
                  .map(parseKidsChampAdminLiveUpdate)
                  .filter((update): update is KidsChampAdminLiveUpdate => update !== null);
                setLiveVersion((current) => current + 1);
                if (!updates.length || updates.some(liveUpdateAffectsParticipants)) {
                  setParticipantRefreshVersion((current) => current + 1);
                }
                if (updates.some(liveUpdateAffectsMerges)) {
                  setMergeRefreshVersion((current) => current + 1);
                }
                if (updates.some((update) => update.entityType === "SETTINGS")) {
  ...
  useEffect(() => {
    const refresh = (event: Event) => {
      ...
      setLiveVersion((current) => current + 1);
      if (localUpdateAffectsParticipants(path)) setParticipantRefreshVersion((current) => current + 1);
      if (localUpdateAffectsMerges(path)) setMergeRefreshVersion((current) => current + 1);
      if (path.endsWith("/kids-champ/settings")) setSettingsRefreshVersion((current) => current + 1);
    };
    window.addEventListener("aplus-data-updated", refresh);
    return () => window.removeEventListener("aplus-data-updated", refresh);
  }, []);
  ...
  useEffect(() => {
    const timer = window.setTimeout(preloadParticipantWorkspace, 1_000);
    window.addEventListener("aplus-auth-cleared", clearParticipantWorkspaceCache);
    return () => {
      window.clearTimeout(timer);
      window.removeEventListener("aplus-auth-cleared", clearParticipantWorkspaceCache);
    };
  }, []);
  ...
  function changeWorkspace(next: Workspace) {
    ...
    startWorkspaceTransition(() => setWorkspace(next));
  ...
  function openOverviewSubmissions(filter: OverviewSubmissionFilter) {
    ...
    startWorkspaceTransition(() => setWorkspace("Submissions"));
  ...
  function openOverviewZip(view: OverviewZipView) {
    ...
    startWorkspaceTransition(() => setWorkspace("ZIP"));
  ...
  return (
    <>
      ...
      {workspaceSwitching ? (
        <div className="fixed inset-0 z-[250] grid place-items-center bg-[#102044]/15 px-4 backdrop-blur-[1px]" role="status" aria-live="polite">
          <div className="flex items-center gap-3 rounded-[16px] border border-white/80 bg-white px-5 py-4 shadow-xl">
            <span className="size-5 animate-spin rounded-full border-[3px] border-blue-100 border-t-[#1689F7]" />
            <div>
              <p className="text-[13px] font-semibold text-[#172A4B]">Opening workspace</p>
              <p className="mt-0.5 text-[10px] text-[#708099]">Preparing the requested Kids Champ data...</p>
            </div>
          </div>
        </div>
      ) : null}

      <main className="mt-6" aria-busy={workspaceSwitching}>
        {workspace === "Overview" ? (
          <Overview
        ...
        {workspace === "Participants" ? (
          <ParticipantsWorkspace
            settings={settings}
            refreshVersion={participantRefreshVersion}
            mergeRefreshVersion={mergeRefreshVersion}
        ...
        {workspace === "Account & Management" ? (
          <FullAccountManagementWorkspace notify={notify} />
        ) : null}
      </main>
