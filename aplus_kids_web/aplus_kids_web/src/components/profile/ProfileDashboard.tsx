...
type ClaimableHistory = { id: string; childName: string; parentName: string; maskedPhone: string; submissionCount: number; firstSubmittedAt: string; lastSubmittedAt: string };
      ...
      <section className="profile-panel mt-5">
        ...
        {claimable.length ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50/70 p-4">
          ...
          <div className="mt-3 space-y-2">{claimable.map((history) => <div key={history.id} className="flex flex-col gap-3 rounded-xl border border-amber-100 bg-white p-3 tablet:flex-row tablet:items-center">
            <div className="min-w-0 flex-1"><p className="text-sm font-medium text-slate-800">{history.childName} · {history.submissionCount} submission{history.submissionCount === 1 ? "" : "s"}</p><p className="mt-1 text-xs text-slate-500">Guardian {history.parentName} · {history.maskedPhone} · last submitted {new Date(history.lastSubmittedAt).toLocaleDateString()}</p></div>
