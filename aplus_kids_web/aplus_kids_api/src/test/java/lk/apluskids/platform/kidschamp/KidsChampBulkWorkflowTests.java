...
class KidsChampBulkWorkflowTests {
    ...
    @Autowired KidsChampWhatsAppTemplateRepository whatsappTemplates;
    ...
    void verifiedAccountCanClaimMatchingGuestHistoryOnceForAnOwnedChild() {
        ...
        var guestId = stored.getGuestParticipant().getPublicId();
    ...
    void persistsSharedSettingsCalendarTasksAndAuditActivity() {
        ...
        var approvedTemplate = new KidsChampWhatsAppTemplateEntity();
        approvedTemplate.synchronize("bulk-workflow-template", "kids_champ_test", "en_US", "UTILITY",
            "APPROVED", "Hello {{1}}, ref {{2}}", List.of("1", "2"));
        whatsappTemplates.save(approvedTemplate);
        var campaign = admin.createCampaign(actor.getPublicId(), "WHATSAPP", "Hello {{1}}, ref {{2}}",
            List.of(participant.id()), "kids_champ_test", "en_US", List.of("{name}", "{reference}"));
