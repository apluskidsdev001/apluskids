...
public class AdministratorManagementController {
    ...
    List<AdministratorView> list(JwtAuthenticationToken auth, @RequestParam(required = false) String search,
        @RequestParam(required = false) AdministratorMembershipStatus status,
        @RequestParam(required = false) AdministratorRole role) {
    ...
    AdministratorView suspend(JwtAuthenticationToken auth, @PathVariable UUID id, @Valid @RequestBody ActionRequest request) {
        return service.suspend(subject(auth), id, new ReasonRequest(request.reason()));
    }

    @PostMapping("/api/v1/admin/administrator-management/{id}/restore")
    AdministratorView restore(JwtAuthenticationToken auth, @PathVariable UUID id, @Valid @RequestBody ActionRequest request) {
        return service.restore(subject(auth), id, new ReasonRequest(request.reason()));
    }

    @PostMapping("/api/v1/admin/administrator-management/{id}/cancel")
    AdministratorView cancel(JwtAuthenticationToken auth, @PathVariable UUID id, @Valid @RequestBody ActionRequest request) {
        return service.cancel(subject(auth), id, new ReasonRequest(request.reason()));
    }

    @PostMapping("/api/v1/admin/administrator-management/{id}/remove")
    AdministratorView remove(JwtAuthenticationToken auth, @PathVariable UUID id, @Valid @RequestBody ActionRequest request) {
        return service.remove(subject(auth), id, new ReasonRequest(request.reason()));
    }

    @PostMapping("/api/v1/admin/administrator-management/{id}/revoke-sessions")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void revokeSessions(JwtAuthenticationToken auth, @PathVariable UUID id, @Valid @RequestBody ActionRequest request) {
        service.revokeAdministratorSessions(subject(auth), id, new ReasonRequest(request.reason()));
    }
    ...
    Map<String, String> validate(@Valid @RequestBody ValidateInvitationRequest request) {
        service.validateInvitationCode(new ValidateAdministratorInvitationRequest(request.email(), request.code()));
        return Map.of("message", "Verification code confirmed.");
    }

    @PostMapping("/api/v1/admin-invitations/resend")
    Map<String, String> resend(@Valid @RequestBody ResendInvitationRequest request) {
        service.resend(request.email());
        return Map.of("message", "If the invitation is pending, a new code has been sent.");
    }

    private UUID subject(JwtAuthenticationToken auth) { return UUID.fromString(auth.getToken().getSubject()); }

    record InviteRequest(@NotBlank @Size(min=2,max=120) String name, @NotBlank @Email @Size(max=254) String email,
        @NotBlank @Size(max=30) String phone, @NotNull AdministratorRole role, @NotBlank @Size(max=600) String reason) {}
    ...
    record AcceptInvitationRequest(@NotBlank @Email String email, @NotBlank @Pattern(regexp="^\\d{6}$") String code,
        @NotBlank @Size(min=8,max=128) String password, @NotBlank String confirmPassword) {}
