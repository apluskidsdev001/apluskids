...
public class KidsChampAdminService {
    ...
    KidsChampAdminService(KidsChampSubmissionRepository submissions,KidsChampBatchRepository batches,KidsChampGuestContactRepository guests,
        KidsChampGuestParticipantRepository guestParticipants,KidsChampParticipantMergeRepository participantMerges,KidsChampWhatsAppPreferenceRepository whatsappPreferences,ChildProfileRepository children,
        ...
        KidsChampWhatsAppDeliveryService whatsappDelivery,
        ...
        this.submissions=submissions;this.batches=batches;this.guests=guests;this.guestParticipants=guestParticipants;this.participantMerges=participantMerges;this.whatsappPreferences=whatsappPreferences;this.children=children;this.audits=audits;this.users=users;this.storage=storage;
        this.settings=settings;this.tasks=tasks;this.campaigns=campaigns;this.messageRecipients=messageRecipients;this.whatsappDelivery=whatsappDelivery;this.notifications=notifications;this.events=events;
    ...
    public List<DuplicateGuestResponse> duplicateGuests(){
        return duplicateGuests(duplicateAnalysis());
    }

    @Transactional(readOnly=true)
    public List<DuplicateGuestResponse> registeredGuestMatches(){
        return registeredGuestMatches(duplicateAnalysis());
    }

    @Transactional(readOnly=true)
    public List<DuplicateGuestResponse> duplicateMatches(){
        DuplicateAnalysis analysis=duplicateAnalysis();
        List<DuplicateGuestResponse> result=new ArrayList<>();
        result.addAll(registeredGuestMatches(analysis));
        result.addAll(duplicateGuests(analysis));
        return result;
    }

    private List<DuplicateGuestResponse> duplicateGuests(DuplicateAnalysis analysis){
        List<KidsChampGuestParticipantEntity> values=analysis.guests();
        List<DuplicateGuestResponse> result=new ArrayList<>();
        for(int i=0;i<values.size();i++) for(int j=i+1;j<values.size();j++){
            ...
            if(analysis.ignoredPairs().contains(pairKey(first,second))) continue;
            var aContact=a.getContact();var bContact=b.getContact();
            List<String> reasons=new ArrayList<>();
            boolean sameChild=KidsChampGuestParticipantEntity.normalize(a.getChildName()).equals(KidsChampGuestParticipantEntity.normalize(b.getChildName()));
            boolean sameDob=a.getDateOfBirth().equals(b.getDateOfBirth());
            if(sameChild)reasons.add("Same child name");if(sameDob)reasons.add("Same date of birth");
            if(same(aContact.getPhoneE164(),bContact.getPhoneE164())) reasons.add("Same guardian mobile number");
            else if(phoneTail(aContact.getPhoneE164()).equals(phoneTail(bContact.getPhoneE164()))) reasons.add("Same final 7 mobile digits");
            if(aContact.getEmail()!=null&&same(aContact.getEmail(),bContact.getEmail())) reasons.add("Same guardian email address");
            if(same(aContact.getParentName(),bContact.getParentName())) reasons.add("Same parent or guardian name");
            ...
            boolean contactMatch=reasons.stream().anyMatch(r->r.contains("mobile")||r.contains("email"));
            boolean strong=sameChild&&(sameDob||contactMatch);
            if(strong) result.add(new DuplicateGuestResponse(a.getPublicId(),b.getPublicId(),a.getChildName(),b.getChildName(),"Guest","Guest",
                aContact.getParentName(),bContact.getParentName(),aContact.getPhoneE164(),bContact.getPhoneE164(),a.getHometown(),b.getHometown(),
                a.getDateOfBirth(),b.getDateOfBirth(),analysis.guestCounts().getOrDefault(a.getPublicId(),0),analysis.guestCounts().getOrDefault(b.getPublicId(),0),reasons,"GUEST_GUEST"));
        }
        return result;
    }

    private List<DuplicateGuestResponse> registeredGuestMatches(DuplicateAnalysis analysis){
        List<DuplicateGuestResponse> result=new ArrayList<>();
        for(var guest:analysis.guests()) for(var accountEntry:analysis.registered().values()){
            UUID childId=accountEntry.getChildProfile().getPublicId(),guestId=guest.getPublicId();var contact=guest.getContact();
            UUID first=first(childId,guestId),second=second(childId,guestId);
            if(analysis.ignoredPairs().contains(pairKey(first,second))) continue;
            List<String> reasons=new ArrayList<>();
            boolean sameChild=KidsChampGuestParticipantEntity.normalize(guest.getChildName()).equals(KidsChampGuestParticipantEntity.normalize(accountEntry.getChildProfile().getFullName()));
            boolean sameDob=guest.getDateOfBirth().equals(accountEntry.getChildProfile().getDateOfBirth());
            if(sameChild)reasons.add("Same child name");if(sameDob)reasons.add("Same date of birth");
            if(same(contact.getPhoneE164(),accountEntry.getUser().getPhoneE164())) reasons.add("Same account mobile number");
            else if(phoneTail(contact.getPhoneE164()).equals(phoneTail(accountEntry.getUser().getPhoneE164()))) reasons.add("Same final 7 mobile digits");
            if(contact.getEmail()!=null&&same(contact.getEmail(),accountEntry.getUser().getEmail())) reasons.add("Same account email address");
            if(same(contact.getParentName(),accountEntry.getUser().getAccountHolderName())) reasons.add("Same parent or account-holder name");
            ...
            boolean strong=sameChild&&(sameDob||contactMatch);
            if(strong){
                int registeredCount=analysis.registeredCounts().getOrDefault(childId,0);
                result.add(new DuplicateGuestResponse(childId,guestId,accountEntry.getChildProfile().getFullName(),guest.getChildName(),"Registered","Guest",
                    accountEntry.getUser().getAccountHolderName(),contact.getParentName(),accountEntry.getUser().getPhoneE164(),contact.getPhoneE164(),
                    accountEntry.getChildProfile().getHometown(),guest.getHometown(),accountEntry.getChildProfile().getDateOfBirth(),guest.getDateOfBirth(),
                    registeredCount,analysis.guestCounts().getOrDefault(guestId,0),reasons,"REGISTERED_GUEST"));
            }
        }
        return result;
    }
    ...
    @Transactional
    public ParticipantMergeResponse mergeGuestIntoRegistered(UUID actorId,UUID childId,UUID guestId,String action,String reason,List<String> matchingReasons){
        requireMergeReason(reason);
        lockedSettings();
        ...
        var guest=guestParticipants.findLockedByPublicId(guestId).orElseThrow(()->bad("GUEST_PARTICIPANT_NOT_FOUND","The guest participant was not found."));
        if(guest.getMergedAt()!=null)throw bad("PARTICIPANT_ALREADY_MERGED","This participant has already been merged.");
        var moved=submissions.findAllByGuestParticipantPublicIdForUpdate(guestId);
        KidsChampParticipantMergeEntity merge=new KidsChampParticipantMergeEntity();merge.create(guest,childId,"REGISTERED",reason.trim(),matchingReasons,moved.size(),actor);participantMerges.save(merge);
        moved.forEach(item->item.mergeIntoRegistered(registered.getUser(),registered.getChildProfile(),merge));guest.markMerged(childId,"REGISTERED",actor,reason.trim());
        audit(actor,action,"CHILD_PROFILE",childId,"Source "+guestId+" merged into registered child. Submissions moved: "+moved.size()+". Reason: "+reason.trim());
        return mergeResponse(merge);
    }

    @Transactional
    public ParticipantMergeResponse mergeGuests(UUID actorId,UUID targetId,UUID sourceId,String reason,List<String> matchingReasons){
        return combineGuests(actorId,targetId,sourceId,"GUEST_RECORDS_MERGED",reason,matchingReasons);
    }

    @Transactional
    public ParticipantMergeResponse deleteDuplicateGuest(UUID actorId,UUID keepId,UUID duplicateId,String reason,List<String> matchingReasons){
        return combineGuests(actorId,keepId,duplicateId,"DUPLICATE_GUEST_MERGED",reason,matchingReasons);
    }

    private ParticipantMergeResponse combineGuests(UUID actorId,UUID targetId,UUID sourceId,String action,String reason,List<String> matchingReasons){
        if(targetId.equals(sourceId)) throw bad("MERGE_TARGET_INVALID","Choose two different guest records.");
        requireMergeReason(reason);
        lockedSettings();
        UserEntity actor=user(actorId);
        var target=guestParticipants.findLockedByPublicId(targetId).orElseThrow(()->bad("GUEST_PARTICIPANT_NOT_FOUND","The destination participant was not found."));
        var source=guestParticipants.findLockedByPublicId(sourceId).orElseThrow(()->bad("GUEST_PARTICIPANT_NOT_FOUND","The source participant was not found."));
        if(target.getMergedAt()!=null||source.getMergedAt()!=null)throw bad("PARTICIPANT_ALREADY_MERGED","A selected participant has already been merged.");
        var moved=submissions.findAllByGuestParticipantPublicIdForUpdate(sourceId);
        KidsChampParticipantMergeEntity merge=new KidsChampParticipantMergeEntity();merge.create(source,targetId,"GUEST",reason.trim(),matchingReasons,moved.size(),actor);participantMerges.save(merge);
        moved.forEach(item->item.mergeIntoGuest(target,merge));source.markMerged(targetId,"GUEST",actor,reason.trim());
        audit(actor,action,"GUEST_PARTICIPANT",targetId,"Source "+sourceId+" merged into destination. Submissions moved: "+moved.size()+". Reason: "+reason.trim());
        return mergeResponse(merge);
    }
    ...
    public List<ParticipantResponse> participants(){
        Map<UUID,String> preferences=whatsappPreferences.findAll().stream().collect(java.util.stream.Collectors.toMap(KidsChampWhatsAppPreferenceEntity::getParticipantReference,KidsChampWhatsAppPreferenceEntity::getStatus));
        Map<String,List<KidsChampSubmissionEntity>> grouped=submissions.findAllByDeletedAtIsNullOrderBySubmittedAtDesc().stream()
            .collect(java.util.stream.Collectors.groupingBy(item->item.getUser()!=null
                ...
                : "guest:"+item.getGuestParticipant().getPublicId(),LinkedHashMap::new,java.util.stream.Collectors.toList()));
        return grouped.values().stream().map(items->{
            ...
            boolean registered=latest.getUser()!=null;var guest=latest.getGuestParticipant();
            Instant fallback=registered?Instant.now():(guest.getCreatedAt()==null?Instant.now():guest.getCreatedAt());
            Instant first=items.stream().map(KidsChampSubmissionEntity::getSubmittedAt).filter(Objects::nonNull).min(Comparator.naturalOrder()).orElse(fallback);
            Instant last=items.stream().map(KidsChampSubmissionEntity::getSubmittedAt).filter(Objects::nonNull).max(Comparator.naturalOrder()).orElse(first);
            UUID id=registered?latest.getChildProfile().getPublicId():guest.getPublicId();
            boolean historicalConsent=items.stream().anyMatch(i->i.getWhatsappConsentAt()!=null);
            LocalDate dob=registered?latest.getChildProfile().getDateOfBirth():guest.getDateOfBirth();
            String name=registered?latest.getChildProfile().getFullName():guest.getChildName();
            String hometown=registered?latest.getChildProfile().getHometown():guest.getHometown();
            String phone=registered?latest.getUser().getPhoneE164():guest.getContact().getPhoneE164();
            String preference=preferences.getOrDefault(id,historicalConsent?"OPTED_IN":"UNKNOWN");
            return new ParticipantResponse(id,name,Period.between(dob,LocalDate.now()).getYears(),dob,registered?"Registered":"Guest",
                hometown,phone,!registered,items.size(),approved,telecasted,first,last,"OPTED_IN".equals(preference),preference);
        }).toList();
    }

    @Transactional
    public WhatsAppPreferenceResponse updateWhatsAppPreference(UUID actorId,UUID participantId,String status,String reason){
        if(status==null||!Set.of("UNKNOWN","OPTED_IN","OPTED_OUT").contains(status))throw bad("WHATSAPP_PREFERENCE_INVALID","Choose unknown, opted in, or opted out.");
        if("OPTED_OUT".equals(status)&&(reason==null||reason.isBlank()))throw bad("WHATSAPP_OPT_OUT_REASON_REQUIRED","Add the opt-out reason before saving.");
        if(participants().stream().noneMatch(item->item.id().equals(participantId)))throw bad("PARTICIPANT_NOT_FOUND","The participant was not found.");
        UserEntity actor=user(actorId);
        KidsChampWhatsAppPreferenceEntity preference=whatsappPreferences.findById(participantId).orElseGet(KidsChampWhatsAppPreferenceEntity::new);
        preference.update(participantId,status,reason==null||reason.isBlank()?null:reason.trim(),actor);whatsappPreferences.save(preference);
        audit(actor,"WHATSAPP_PREFERENCE_UPDATED","PARTICIPANT",participantId,status+(reason==null||reason.isBlank()?"":". Reason: "+reason.trim()));
        return new WhatsAppPreferenceResponse(participantId,status);
    }
    ...
    public ParticipantResponse updateParticipant(UUID actorId,UUID participantId,String name,LocalDate dateOfBirth,String hometown,String phone){
        requireParticipantFields(name,dateOfBirth,hometown);
        lockedSettings();
        UserEntity actor=user(actorId);
        Optional<KidsChampGuestParticipantEntity> guestMatch=guestParticipants.findLockedByPublicId(participantId);
        if(guestMatch.isPresent()){
            KidsChampGuestParticipantEntity guest=guestMatch.get();
            if(guest.getMergedAt()!=null)throw bad("PARTICIPANT_ALREADY_MERGED","Undo this merge before editing the source participant.");
            String normalizedPhone=phone==null?guest.getContact().getPhoneE164():phone.replaceAll("[\\s()-]","");
            if(!normalizedPhone.matches("^\\+[1-9]\\d{7,14}$"))throw bad("PHONE_INVALID","Enter a valid international mobile number.");
            guests.findByPhoneE164(normalizedPhone).filter(existing->!existing.getId().equals(guest.getContact().getId()))
                .ifPresent(existing->{throw bad("PHONE_ALREADY_USED","That mobile number belongs to another guardian record. Merge the records instead of overwriting it.");});
            guest.setChildName(name.trim());guest.setDateOfBirth(dateOfBirth);guest.setHometown(hometown.trim());guest.getContact().setPhoneE164(normalizedPhone);
            audit(actor,"PARTICIPANT_UPDATED","GUEST_PARTICIPANT",participantId,"Updated guest child name, birth date, hometown, and guardian mobile number.");
        }else{
            ChildProfileEntity child=children.findByPublicIdAndDeletedAtIsNull(participantId)
                .orElseThrow(()->bad("PARTICIPANT_NOT_FOUND","The participant was not found."));
            child.setFullName(name.trim());child.setDateOfBirth(dateOfBirth);child.setHometown(hometown.trim());
            audit(actor,"PARTICIPANT_UPDATED","CHILD_PROFILE",participantId,"Updated registered child name, birth date, and hometown.");
        }
        return participants().stream().filter(item->item.id().equals(participantId)).findFirst()
            .orElseThrow(()->bad("PARTICIPANT_NOT_FOUND","The participant has no active Kids Champ submissions."));
    }

    @Transactional(readOnly=true)
    public List<ParticipantMergeResponse> participantMergeHistory(){return participantMerges.findAllByOrderByMergedAtDesc().stream().map(this::mergeResponse).toList();}

    @Transactional
    public ParticipantMergeResponse undoParticipantMerge(UUID actorId,UUID mergeId,String reason){
        requireMergeReason(reason);
        lockedSettings();
        UserEntity actor=user(actorId);
        KidsChampParticipantMergeEntity merge=participantMerges.findLockedByPublicId(mergeId)
            .orElseThrow(()->bad("PARTICIPANT_MERGE_NOT_FOUND","The participant merge record was not found."));
        if(merge.getUndoneAt()!=null)throw bad("PARTICIPANT_MERGE_ALREADY_UNDONE","This merge has already been undone.");
        KidsChampGuestParticipantEntity source=guestParticipants.findLockedByPublicId(merge.getSource().getPublicId())
            .orElseThrow(()->bad("GUEST_PARTICIPANT_NOT_FOUND","The source participant was not found."));
        List<KidsChampSubmissionEntity> moved=submissions.findAllByLastParticipantMergePublicIdForUpdate(mergeId);
        moved.forEach(item->item.restoreGuest(source));source.undoMerge();merge.undo(actor,reason.trim());
        audit(actor,"PARTICIPANT_MERGE_UNDONE","GUEST_PARTICIPANT",source.getPublicId(),"Restored "+moved.size()+" submissions. Reason: "+reason.trim());
        return mergeResponse(merge);
    }

    private void requireParticipantFields(String name,LocalDate dateOfBirth,String hometown){
        if(name==null||name.isBlank()||name.trim().length()>120)throw bad("PARTICIPANT_NAME_INVALID","Enter a participant name up to 120 characters.");
        if(dateOfBirth==null||dateOfBirth.isAfter(LocalDate.now())||Period.between(dateOfBirth,LocalDate.now()).getYears()>17)
            throw bad("PARTICIPANT_AGE_INVALID","Kids Champ participants must be under 18 years old.");
        if(hometown==null||hometown.isBlank()||hometown.trim().length()>120)throw bad("PARTICIPANT_HOMETOWN_INVALID","Enter a hometown up to 120 characters.");
    }

    private void requireMergeReason(String reason){if(reason==null||reason.isBlank()||reason.trim().length()>600)throw bad("MERGE_REASON_REQUIRED","Enter a clear merge reason up to 600 characters.");}

    private ParticipantMergeResponse mergeResponse(KidsChampParticipantMergeEntity merge){
        KidsChampGuestParticipantEntity source=merge.getSource();
        String targetName="Unknown participant";
        if("REGISTERED".equals(merge.getTargetType()))targetName=children.findByPublicIdAndDeletedAtIsNull(merge.getTargetReference()).map(ChildProfileEntity::getFullName).orElse("Deleted registered profile");
        else targetName=guestParticipants.findByPublicId(merge.getTargetReference()).map(KidsChampGuestParticipantEntity::getChildName).orElse("Deleted guest participant");
        List<String> matching=merge.getMatchingReasons()==null||merge.getMatchingReasons().isBlank()?List.of():Arrays.asList(merge.getMatchingReasons().split("\\n"));
        return new ParticipantMergeResponse(merge.getPublicId(),source.getPublicId(),source.getChildName(),"Guest",merge.getTargetReference(),targetName,
            "REGISTERED".equals(merge.getTargetType())?"Registered":"Guest",merge.getReason(),matching,merge.getMovedSubmissionCount(),
            merge.getMergedBy()==null?"System":merge.getMergedBy().getAccountHolderName(),merge.getMergedAt(),merge.getUndoneAt(),
            merge.getUndoneBy()==null?null:merge.getUndoneBy().getAccountHolderName(),merge.getUndoReason());
    }
    ...
    public List<GrowthResponse> growth(){
        ...
        return java.util.stream.IntStream.rangeClosed(0,6).mapToObj(offset->{LocalDate date=today.minusDays(6-offset);
            ...
            long unique=day.stream().map(i->i.getUser()!=null?"u"+i.getChildProfile().getPublicId():"g"+i.getGuestParticipant().getPublicId()).distinct().count();
    ...
    public CampaignResponse createCampaign(UUID actorId,String channel,String template,List<UUID> participantIds){
        return createCampaign(actorId,channel,template,participantIds,null,null,null,null,null);
    }
    ...
    public CampaignResponse createCampaign(UUID actorId,String channel,String template,List<UUID> participantIds,String templateName,String languageCode,List<String> templateParameters){
        return createCampaign(actorId,channel,template,participantIds,templateName,languageCode,templateParameters,null,null);
    }
    ...
    public CampaignResponse createCampaign(UUID actorId,String channel,String template,List<UUID> participantIds,String templateName,String languageCode,List<String> templateParameters,String campaignName,String source){
        ...
        if("WHATSAPP".equals(channel)&&(templateName==null||templateName.isBlank()||languageCode==null||languageCode.isBlank()))throw bad("WHATSAPP_TEMPLATE_REQUIRED","Choose an approved WhatsApp template.");
        ...
        UserEntity actor=user(actorId);KidsChampMessageCampaignEntity campaign=new KidsChampMessageCampaignEntity();campaign.create(channel,template.trim(),selected.size(),actor,campaignName,source,templateName,languageCode);campaigns.save(campaign);
        for(ParticipantResponse participant:selected){
            ...
            String destination=channel.equals("WHATSAPP")?KidsChampWhatsAppAdminService.normalizeSriLanka(participant.phone()):submissionContactEmail(participant.id());
            ...
            List<String> resolvedParameters=(templateParameters==null?List.<String>of():templateParameters).stream().map(value->value.replace("{name}",participant.name()).replace("{reference}",participant.id().toString())).toList();
            String rendered=template.replace("{name}",participant.name()).replace("{reference}",participant.id().toString());
            for(int index=0;index<resolvedParameters.size();index++)rendered=rendered.replace("{{"+(index+1)+"}}",resolvedParameters.get(index));
            KidsChampMessageRecipientEntity recipient=new KidsChampMessageRecipientEntity();recipient.create(campaign,participant.id(),participant.name(),destination,rendered,templateName,languageCode,resolvedParameters);messageRecipients.save(recipient);
        }
    ...
    @Transactional public void deleteRecipients(UUID actorId,List<Long> ids){if(ids==null||ids.isEmpty()||ids.size()>500)throw bad("RECIPIENT_SELECTION_INVALID","Select between 1 and 500 failed messages.");UserEntity actor=user(actorId);for(Long id:ids){KidsChampMessageRecipientEntity recipient=messageRecipients.findById(id).orElseThrow(()->bad("RECIPIENT_NOT_FOUND","Message recipient not found."));try{recipient.delete();}catch(IllegalStateException error){throw bad("RECIPIENT_ACTION_INVALID",error.getMessage());}whatsappDelivery.recordAction(recipient,"DELETED","The failed message was removed from the active queue.");audit(actor,"WHATSAPP_RECIPIENT_DELETED","MESSAGE_RECIPIENT",recipient.getCampaign().getPublicId(),recipient.getParticipantName());}}
    private void messageRecipientAction(UUID actorId,List<Long> ids,boolean retry){if(ids==null||ids.isEmpty()||ids.size()>500)throw bad("RECIPIENT_SELECTION_INVALID","Select between 1 and 500 failed messages.");UserEntity actor=user(actorId);for(Long id:ids){KidsChampMessageRecipientEntity recipient=messageRecipients.findById(id).orElseThrow(()->bad("RECIPIENT_NOT_FOUND","Message recipient not found."));try{if(retry)recipient.retry();else recipient.skip();}catch(IllegalStateException error){throw bad("RECIPIENT_ACTION_INVALID",error.getMessage());}whatsappDelivery.recordAction(recipient,retry?"QUEUED":"SKIPPED",retry?"The message was queued for another delivery attempt.":"The failed message was ignored by an administrator.");audit(actor,retry?"WHATSAPP_RETRY_QUEUED":"WHATSAPP_RETRY_IGNORED","MESSAGE_RECIPIENT",recipient.getCampaign().getPublicId(),recipient.getParticipantName());}}
    private CampaignResponse campaignResponse(KidsChampMessageCampaignEntity campaign){
        List<KidsChampMessageRecipientEntity> values=messageRecipients.findAllByCampaignPublicIdOrderByIdAsc(campaign.getPublicId());
        long queued=values.stream().filter(value->"QUEUED".equals(value.getStatus())).count();long sending=values.stream().filter(value->"SENDING".equals(value.getStatus())).count();
        long accepted=values.stream().filter(value->"SENT".equals(value.getStatus())).count();long delivered=values.stream().filter(value->"DELIVERED".equals(value.getStatus())).count();long read=values.stream().filter(value->"READ".equals(value.getStatus())).count();
        long failed=values.stream().filter(value->"FAILED".equals(value.getStatus())).count();long ignored=values.stream().filter(value->"SKIPPED".equals(value.getStatus())||"DELETED".equals(value.getStatus())).count();
        return CampaignResponse.from(campaign,queued,sending,accepted,delivered,read,failed,ignored);
    }
    ...
    private DuplicateAnalysis duplicateAnalysis(){
        List<KidsChampGuestParticipantEntity> guestValues=guestParticipants.findAllByMergedAtIsNullOrderByUpdatedAtDesc();
        List<KidsChampSubmissionEntity> all=submissions.findAllForParticipantMatching();
        Map<UUID,KidsChampSubmissionEntity> registered=new LinkedHashMap<>();
        Map<UUID,Integer> registeredCounts=new HashMap<>();
        Map<UUID,Integer> guestCounts=new HashMap<>();
        for(var item:all){
            if(item.getGuestParticipant()!=null){
                guestCounts.merge(item.getGuestParticipant().getPublicId(),1,Integer::sum);
            }else if(item.getChildProfile()!=null){
                UUID childId=item.getChildProfile().getPublicId();
                registeredCounts.merge(childId,1,Integer::sum);
                if(item.getDeletedAt()==null)registered.putIfAbsent(childId,item);
            }
        }
        Set<String> ignoredPairs=ignoredGuestMatches.findAll().stream()
            .map(item->pairKey(item.getFirstGuestId(),item.getSecondGuestId()))
            .collect(java.util.stream.Collectors.toSet());
        return new DuplicateAnalysis(guestValues,registered,registeredCounts,guestCounts,ignoredPairs);
    }

    private String pairKey(UUID a,UUID b){return first(a,b)+":"+second(a,b);}
    private record DuplicateAnalysis(List<KidsChampGuestParticipantEntity> guests,Map<UUID,KidsChampSubmissionEntity> registered,
        Map<UUID,Integer> registeredCounts,Map<UUID,Integer> guestCounts,Set<String> ignoredPairs){}
    ...
    public record DuplicateGuestResponse(UUID firstId,UUID secondId,String firstName,String secondName,String firstType,String secondType,
        String firstParentName,String secondParentName,String firstPhone,String secondPhone,String firstHometown,String secondHometown,
        LocalDate firstDateOfBirth,LocalDate secondDateOfBirth,int firstSubmissions,int secondSubmissions,List<String> reasons,String matchType){}
    public record ParticipantResponse(UUID id,String name,int age,LocalDate dateOfBirth,String type,String location,String phone,boolean phoneEditable,
        long submissions,long approved,long telecasted,Instant joinedAt,Instant lastSubmissionAt,boolean whatsappConsented,String whatsappConsentStatus){}
    public record WhatsAppPreferenceResponse(UUID participantId,String status){}
    public record ParticipantMergeResponse(UUID id,UUID sourceId,String sourceName,String sourceType,UUID targetId,String targetName,String targetType,
        String reason,List<String> matchingReasons,int movedSubmissions,String mergedBy,Instant mergedAt,Instant undoneAt,String undoneBy,String undoReason){}
    ...
    public record CampaignResponse(UUID id,String channel,String status,int recipientCount,String messageTemplate,String name,String source,String templateName,String languageCode,long queuedCount,long sendingCount,long acceptedCount,long deliveredCount,long readCount,long failedCount,long ignoredCount,Instant createdAt,Instant completedAt){
        static CampaignResponse from(KidsChampMessageCampaignEntity e,long queued,long sending,long accepted,long delivered,long read,long failed,long ignored){return new CampaignResponse(e.getPublicId(),e.getChannel(),e.getStatus(),e.getRecipientCount(),e.getMessageTemplate(),e.getName(),e.getSource(),e.getTemplateName(),e.getLanguageCode(),queued,sending,accepted,delivered,read,failed,ignored,e.getCreatedAt(),e.getCompletedAt());}}
    public record MessageRecipientResponse(Long id,UUID participantId,String name,String destination,String status,int attempts,String failureReason,Instant sentAt,Instant lastAttemptAt,Instant nextAttemptAt,Instant deliveredAt,Instant readAt){
        static MessageRecipientResponse from(KidsChampMessageRecipientEntity e){return new MessageRecipientResponse(e.getId(),e.getParticipantReference(),e.getParticipantName(),e.getDestination(),e.getStatus(),e.getAttempts(),e.getFailureReason(),e.getSentAt(),e.getLastAttemptAt(),e.getNextAttemptAt(),e.getDeliveredAt(),e.getReadAt());}}
}
